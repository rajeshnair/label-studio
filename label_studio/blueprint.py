...
def config_from_file():
    try:
        ...
        raise LabelStudioError(
            "Can't open input_args file: " + str(INPUT_ARGUMENTS_PATH) + ", "
...
def project_get_or_create(multi_session_force_recreate=False):
    ...
    if input_args and input_args.command == "start-multi-session":
        ...
        return Project.get_or_create(
            project_name,
            input_args,
            context={"multi_session": True, "user": convert_string_to_hash(user)},
        )
    else:
        ...
        return Project.get_or_create(
            input_args.project_name, input_args, context={"multi_session": False}
        )
...
def get_data_file(filename):
    ...
    if not g.project.config.get("allow_serving_local_files"):
        raise FileNotFoundError(
            "Serving local files is not allowed. "
...
def labeling_page():
    ...
    if task_id is not None:
        ...
        task_data = g.project.get_task_with_completions(
            task_id
        ) or g.project.source_storage.get(task_id)
...
def setup_page():
    ...
    if project.config.get("show_project_links_in_multisession", True) and hasattr(
        g, "user"
    ):
        user = g.user
...
def import_page():
    ...
    return flask.render_template(
        "import.html", config=g.project.config, project=g.project
    )
        ...
        if ml_backend.connected:
            try:
                ...
                try:
                    ...
                    ml_backend.error = {
                        "detail": "Can't parse exception message from ML Backend"
                    }
...
def api_render_label_studio():
    ...
    response = {"label_config_line": config_line, "task_ser": example_task_data}
...
def api_validate_config():
    ...
    if "label_config" not in request.form:
        return make_response(
            "No label_config in POST", status.HTTP_417_EXPECTATION_FAILED
        )
    try:
        g.project.validate_label_config(request.form["label_config"])
    except ValidationError as e:
        return make_response(
            jsonify({"label_config": e.msg_to_list()}), status.HTTP_400_BAD_REQUEST
        )
    except Exception as e:
        return make_response(
            jsonify({"label_config": [str(e)]}), status.HTTP_400_BAD_REQUEST
        )
...
def api_import_example_file():
    ...
    if q == "json":
        ...
        filename += ".tsv"
        output = pd.read_json(json.dumps(tasks), orient="records").to_csv(
            index=False, sep="\t"
        )
...
def api_import():
    ...
    * url links to images, audio, csv (if you use TimeSeries in labeling config)
    ...
    try:
        ...
        raise NotImplementedError(
            "Import is not supported for the current storage "
            + str(g.project.source_storage)
        )
    ...
    if request.method == "GET":
        ...
        for storage_for in all_forms:
            for name, description in g.project.get_available_storage_names(
                ...
                form = (
                    form_class(data=g.project.get_storage(storage_for).get_params())
                    if current
                    else form_class()
                )
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                return make_response(
                    jsonify({"detail": "Error while storage update: " + str(e)}), 400
                )
            else:
                return make_response(jsonify({"result": "ok"}), 201)
        else:
            logger.error(
                "Errors: " + str(form.errors) + " for request body " + str(request.json)
            )
...
def api_all_tasks():
    ...
    if request.method == "GET":
        ...
        params = SimpleNamespace(
            fields=fields, page=page, page_size=page_size, order=order
        )
...
def api_tasks_completions(task_id):
    ...
    if request.method == "DELETE":
        if g.project.config.get("allow_delete_completions", False):
            ...
            return make_response(
                {"detail": "Completion removing is not allowed in server config"}, 422
            )


@blueprint.route(
    "/api/tasks/<task_id>/completions/<completion_id>", methods=["PATCH", "DELETE"]
)
@requires_auth
@exception_handler
...
def api_train():
    ...
    if g.project.ml_backends_connected:
        ...
        if training_started:
            ...
            return make_response(
                jsonify(
                    "Training is not started: seems that you don't have any ML backend connected"
                ),
                400,
            )
    else:
    ...
    if g.project.ml_backends_connected:
        ...
        if mode == "data":
            if request.json is None:
                return make_response(
                    jsonify({"detail": "no task data found in request json"}), 422
                )
        ...
        # make prediction for all tasks
            ...
            g.project.source_storage.set_many(
                tasks_with_predictions.keys(), tasks_with_predictions.values()
            )
...
def main():
    ...
    if input_args.command == "start":
        ...
        label_studio.utils.auth.USERNAME = (
            ...
            or label_studio.utils.auth.USERNAME
        )
        ...
        server_host = (
            "localhost" if host == "localhost" else "0.0.0.0"
        )  # web server host
        ...
        if input_args.use_gevent:
            ...
            ssl_args = (
                {"keyfile": key_file, "certfile": cert_file} if ssl_context else {}
            )
            http_server = WSGIServer(
                (server_host, port), app, log=app.logger, **ssl_args
            )
