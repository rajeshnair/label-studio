...
def action_config(test_client, case_config):
    ...
    data = {"label_config": case_config["label_config"]}
...
def action_label(test_client, case_config):
    ...
    response = test_client.post(
        "api/tasks/{task_id}/completions/".format(task_id=task_id),
...
def action_label_test(test_client, case_config):
    ...
    filename = os.path.join(
        project.config.get("output_dir", None), "{task_id}.json".format(task_id=task_id)
    )
    with open(filename) as json_file:
        completion = json.load(json_file)
        assert (
            completion.get("completions", {})[0].get("result", [])
            == completion["result"]
        )
...
def action_get_change_completion(test_client, case_config):
    ...
    response = test_client.get(
        "/api/tasks/{task_id}/completions/{completion_id}/".format(
...
def action_export(test_client, case_config):
    ...
    response = test_client.get(
        "/api/project/export?format={export_format}".format(export_format=export_format)
    )
    assert response.status_code == 200
