...
class LabelStudioMLManager(object):
    ...
    def _get_latest_job_result_from_workdir(cls, project):
        ...
        for subdir in reversed(
            sorted(
                map(int, filter(lambda d: d.isdigit(), os.listdir(project_model_dir)))
            )
        ):
            ...
            if not os.path.exists(job_result_file):
                logger.error(
                    "The latest job result file " + job_result_file + " doesn't exist"
                )
                continue
    ...
    @classmethod
    def create(
        cls, project=None, label_config=None, train_output=None, version=None, **kwargs
    ):
        key = cls._key(project)
        ...
        cls._current_model[key] = ModelWrapper(
            model=cls.model_class(
                label_config=label_config, train_output=train_output, **kwargs
            ),
    ...
    def get_or_create(
        ...
        **kwargs
        ...
        if (
            ...
            or force_reload
            or (cls.get(project).model_version != version and version is not None)
        ):  # noqa
            logger.debug(
                "Reload model for project={project} with version={version}".format(
    ...
    def fetch(cls, project=None, label_config=None, force_reload=False, **kwargs):
        ...
        return cls.get_or_create(
            project, label_config, force_reload, train_output, version, **kwargs
        )
    ...
    def _start_training_job(cls, project, label_config, train_kwargs):
        ...
        logger.info(
            "Training job {job} started for project {project}".format(
    ...
    def train(cls, tasks, project=None, label_config=None, **kwargs):
        ...
        if cls.without_redis():
            ...
            cls.get_or_create(
                project, label_config, force_reload=True, train_output=train_output
            )
        else:
