...
def get_args():
    ...
    root_parser.add_argument(
        "--root-dir", dest="root_dir", default=".", help="Projects root directory"
    )
...
def create_dir(args):
    ...
    shutil.copytree(
        default_configs_dir, output_dir, ignore=shutil.ignore_patterns("*.tmpl")
    )
    ...
    wsgi_script = wsgi_script.format(
        script=os.path.splitext(script_base_name)[0], model_class=model_class
    )
    ...
    print(
        Fore.GREEN
        + "Congratulations! ML Backend has been successfully initialized in "
        + output_dir
    )
    print(
        Fore.RESET
