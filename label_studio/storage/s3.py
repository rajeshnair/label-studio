...
class S3StorageForm(CloudStorageForm):
    ...
    bound_params = dict(region="region", **BaseStorageForm.bound_params)
...
class S3Storage(CloudStorage):
    ...
    def _get_client(self):
        ...
        return {"s3": s3, "client": client, "bucket": s3.Bucket(self.path)}

    def get_params(self):
        """Get params to fill the form"""
        params = super(S3Storage, self).get_params()
        params.update({"region": self.region})
        return params
    ...
    def _get_objects(self):
        bucket = self.client["bucket"]
        if self.prefix:
            bucket_iter = bucket.objects.filter(
                Prefix=self.prefix.rstrip("/") + "/", Delimiter="/"
            ).all()
        else:
...
class S3CompletionsStorageForm(BaseStorageForm):
    ...
    create_local_copy = BooleanField(
        "Create local copy",
        description="Create a local copy on your disk",
        default=True,
    )
...
class S3CompletionsStorage(S3Storage):
    ...
    def __init__(self, use_blob_urls=False, regex=".*", **kwargs):
        ...
        super(S3CompletionsStorage, self).__init__(
            use_blob_urls=False, regex=".*", **kwargs
        )
