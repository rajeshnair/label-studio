...
def parse_input_args():
    ...
    root_parser.add_argument(
        "--root-dir", dest="root_dir", default=".", help="Project root directory"
    )
    root_parser.add_argument(
        "-v",
        "--verbose",
    ...
    root_parser.add_argument(
        "-c", "--config", dest="config_path", type=valid_filepath, help="Server config"
    )
    root_parser.add_argument(
        "-l",
        "--label-config",
    ...
    root_parser.add_argument(
        "--source-path", dest="source_path", help="Source bucket name"
    )
    root_parser.add_argument(
        "--source-params",
        dest="source_params",
    ...
    root_parser.add_argument(
        "--target-path", dest="target_path", help="Target bucket name"
    )
    root_parser.add_argument(
        "--target-params",
        dest="target_params",
    ...
    root_parser.add_argument(
        "--protocol", dest="protocol", type=str, help="Web protocol http:// or https://"
    )
    root_parser.add_argument("-p", "--port", dest="port", type=int, help="Server port")
    root_parser.add_argument(
        "--cert",
        dest="cert_file",
        type=valid_filepath,
    ...
    parser_init.add_argument(
        "project_name", help="Path to directory where project state will be initialized"
    )
    ...
    parser_start.add_argument(
        "--password", dest="password", default="", help="Password for web access"
    )
    parser_start.add_argument(
        "--username", dest="username", default="", help="Username for web access"
    )
    ...
    label_config_explicitly_specified = (
        hasattr(args, "label_config") and args.label_config
    )
    if args.template and not label_config_explicitly_specified:
        args.label_config = os.path.join(
            find_dir("examples"), args.template, "config.xml"
        )
