    ...
    def validate_label_config(cls, config_string):
        ...
        try:
            ...
            error_message = "Validation failed on {}: {}".format(
                "/".join(exc.path), error_message.replace("@", "")
            )
        ...
        if len(set(all_names)) != len(all_names):
            ...
            raise ValidationError(
                "Label config contains non-unique names " + str(all_names)
            )
        ...
        for toName_ in toNames:
            for toName in toName_.split(","):
                if toName not in names:
                    raise ValidationError(
                        'toName="{toName}" not found in names: {names}'.format(
                            toName=toName, names=sorted(names)
                        )
                    )
...
class MLApi(BaseHTTPAPI):
    ...
    def _get(self, url_suffix, *args, **kwargs):
        ...
        try:
            ...
            error_msg = (
                response.content.decode("utf-8") if response is not None else str(e)
            )
            return MLApiResult(
                url, {}, {"error": error_msg}, headers, "error", status_code=status_code
            )
        ...
        try:
            ...
            return MLApiResult(
                url,
                {},
                {"error": str(e), "response": response.content.decode("utf-8")},
                headers,
                "error",
    ...
    def delete(self, project):
        ...
        """
        return self._post(
            "delete", request={"project": self._create_project_uid(project)}
        )
...
class MLBackend(object):
    ...
    def restore_train_job(self):
        ...
        if not os.path.exists(self._TRAIN_JOBS_FILE):
            logger.warning(
                "Can't restore train job because "
                + self._TRAIN_JOBS_FILE
                + " not found"
            )
            return
    ...
    def train_job_is_running(self):
        if self._api_exists() and self.train_job is not None:
            ...
            if response.is_error:
                logger.error(
                    "Can't fetch train job status for job " + self.train_job + ": "
    ...
    def _api_exists(self):
        if self.api is None or not self.api.is_ok():
            logger.debug(
                "Can't make predictions because ML backend was not specified: "
    ...
    def make_predictions(self, task, project):
        ...
        if response.is_error:
            if response.status_code == 404:
                logger.info(
                    "Can't make predictions: model is not found (probably not trained yet)"
                )
            else:
                logger.error(
                    "Can't make predictions: ML backend returns an error: "
    ...
    def is_training(self, project):
        if self._api_exists():
            ...
            if response.is_error:
                raise CantValidateIsTraining(
                    "Can't validate whether model is training for project "
                    + project.name
                )
    ...
    def train(self, completions, project):
        if self.train_job_is_running():
            raise CantStartTrainJobError(
                "Can't start new training: Train job is running."
            )
        ...
        if response.is_error:
            raise CantStartTrainJobError(
                "Can't update model: ML backend returns an error: "
                + response.error_message
            )
        else:
            ...
            logger.info(
                "Training job with "
                + str(self.num_completions)
                + " completions has been started."
            )
            ...
            if maybe_job:
                ...
                logger.debug(
                    "Project "
                    + str(project)
                    + " successfully updated train job "
                    + self.train_job
                )
        ...
        if self._api_exists():
            ...
            if response.is_error:
                if response.status_code == 422:
                    raise ValidationError(
                        "ML backend doesn't accept current label config "
                        + label_config
                    ...
                    raise ValueError(
                        "Can't infer schema for label config "
                        + label_config
                        + " ML backend returns error: "
                        + response.error_message
                    )
            else:
                return response.response
    ...
    def get_schema(self, project):
        ...
        if len(schema) > 1:
            logger.warning(
                "ML backend returns multiple schemas for label config "
    ...
    def clear(self, project):
        if self._api_exists():
            ...
            if response.is_error:
                logger.error(
                    "Can't clear ML backend for project "
                    + project.name
                    + ": "
                    + response.error_message
                )
                ...
                logger.info(
                    "ML backend for project " + project.name + " has been cleared."
                )
