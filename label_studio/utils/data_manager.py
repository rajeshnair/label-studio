    ...
    pre_order = (
        {
            ...
            "has_cancelled_completions": cancelled_status[i]
    ...
    if order == "id":
        ...
        if order == "has_cancelled_completions":
            ordered = sorted(
                pre_order,
                key=lambda x: (
                    DirectionSwitch(x["has_cancelled_completions"], not ascending),
        ...
        # another orderings
            ...
            ordered = sorted(
                pre_order, key=lambda x: (DirectionSwitch(x[order], not ascending))
            )
    ...
    for item in paginated:
        ...
        if task is None:
            ...
            if completed_at != "undefined" and completed_at is not None:
                completed_at = timestamp_to_local_datetime(completed_at).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
