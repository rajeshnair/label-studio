...
def tasks_from_file(filename, file, project):
    ...
    try:
        if filename.endswith(".csv") and not is_time_series_only(project):
            ...
            tasks = [
                {"data": {settings.UPLOAD_DATA_UNDEFINED_NAME: line.decode("utf-8")}}
                for line in lines
            ]
            file_format = os.path.splitext(filename)[-1]
        ...
        raise ValidationError(
            "Failed to parse input file " + filename + ": " + str(exc)
        )
...
def extract_archive(archive, filename, temp_dir):
    ...
    logger.info(
        "ZIP archive {filename} found with {names} files inside, extracting to {final_dir}".format(
            filename=filename, names=len(names), final_dir=final_dir
        )
    )
...
def check_max_task_number(tasks):
    ...
    if len(tasks) > settings.TASKS_MAX_NUMBER:
        raise ValidationError(
            "Maximum task number is {TASKS_MAX_NUMBER}, "
            "current task number is {num_tasks}".format(
                TASKS_MAX_NUMBER=settings.TASKS_MAX_NUMBER, num_tasks=len(tasks)
            )
        )
...
def check_file_sizes_and_number(files):
    ...
    if total >= settings.TASKS_MAX_FILE_SIZE:
        raise ValidationError(
            "Maximum total size of all files is {TASKS_MAX_FILE_SIZE} bytes, "
            "current size is {total} bytes".format(
                TASKS_MAX_FILE_SIZE=settings.TASKS_MAX_FILE_SIZE, total=total
            )
        )
