...
class Project(object):
    ...
    def create_storages(self):
        ...
        self.source_storage = create_storage(
            source["type"],
            "source",
            source["path"],
            self.path,
        ...
        self.target_storage = create_storage(
            target["type"],
            "target",
            target["path"],
            self.path,
    ...
    def add_ml_backend(self, params, raise_on_error=True):
        ...
        if not ml_backend.connected and raise_on_error:
            raise ValueError(
                'ML backend with URL: "' + str(params["url"]) + '" is not connected.'
            )
    ...
    def remove_ml_backend(self, name):
        # remove from memory
        remove_idx = next(
            (i for i, b in enumerate(self.ml_backends) if b.model_name == name), None
        )
        if remove_idx is None:
            raise KeyError(
                "Can't remove ML backend with name \"" + name + '": not found.'
            )
        ...
        remove_idx = next(
            (i for i, b in enumerate(config_params) if b["name"] == name), None
        )
        ...
        for item in input_values:
            if item not in self.derived_input_schema:
                raise ValidationError(
                    ...
                    "You've specified value=${item}, but imported tasks contain only keys: {input_schema_values}".format(
                        item=item, input_schema_values=list(self.derived_input_schema)
                    )
                )
        ...
        for from_name, to_name, type in output_schema["from_name_to_name_type"]:
            if (from_name, to_name, type) not in completion_tuples:
                raise ValidationError(
                    ...
                    "name={from_name}, toName={to_name}, type={type} are expected".format(
                        from_name=from_name, to_name=to_name, type=type
                    )
                )
            ...
            if extra_labels:
                raise ValidationError(
                    ...
                    'there are labels already created for "{from_name}":\n{extra_labels}'.format(
                        from_name=from_name, extra_labels=extra_labels
                    )
                )
    ...
    def next_task(self, completed_tasks_ids):
        ...
        task_iter = filter(
            lambda i: i not in completed_tasks_ids, sorted(self.source_storage.ids())
        )
        ...
        # Task with minimum / maximum average prediction score is taken
            ...
            for task_id, task in self.source_storage.items():
                ...
                if "predictions" in task and len(task["predictions"]) > 0:
                    score = sum((p["score"] for p in task["predictions"]), 0) / len(
                        task["predictions"]
                    )
    ...
    def get_completions_ids(self):
        ...
        logger.debug(
            "{num} completions found in {output_dir}".format(
    ...
    def get_completed_at(self):
        ...
        for _, data in self.target_storage.items():
            id = data["id"]
            try:
                times[id] = max(data["completions"], key=itemgetter("created_at"))[
                    "created_at"
                ]
    ...
    def _load_tasks(cls, input_path, args, label_config_file):
        ...
        if len(input_data_tags) > 1:
            ...
            print(
                "Warning! Multiple input data tags found: "
                + val
        ...
        raise RuntimeError(
            "Can't load tasks for input format={}".format(args.input_format)
        )
    ...
    def create_project_dir(cls, project_name, args):
        ...
        config = (
            json_load(args.config_path)
            if args.config_path
            else json_load(find_file("default_config.json"))
        )
        ...
        if label_config_file:
            ...
            if not input_path:
                ...
                print(
                    default_label_config + " label config copied to " + config_xml_path
                )
            else:
        ...
        if args.source:
            ...
            logger.debug(
                "{tasks_json_path} input file with {n} tasks has been created from {input_path}".format(
        ...
        if (
            hasattr(args, "web_gui_project_desc") and args.web_gui_project_desc
        ) or args.project_desc:
    ...
    def _get_config(cls, project_dir, args=None):
        ...
        if config.get("input_path"):
            config["input_path"] = os.path.join(
                os.path.dirname(config_path), config["input_path"]
            )
        config["label_config"] = os.path.join(
            os.path.dirname(config_path), config["label_config"]
        )
        if config.get("output_dir"):
            config["output_dir"] = os.path.join(
                os.path.dirname(config_path), config["output_dir"]
            )
    ...
    def serialize(self):
        ...
        available_storages = list(
            filter(
                lambda i: i[0] not in ban_list, get_available_storage_names().items()
            )
        )
